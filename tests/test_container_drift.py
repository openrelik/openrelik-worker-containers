...
from src.container_drift import container_drift
from src.container_drift import read_container_explorer_output


class TestContainerDriftTask(unittest.TestCase):
    """Unit test for OpenRelik container drift."""
    ...
    def setUp(self):
        ...
        self.log_file_path = os.path.join(self.output_path, "container_drift.log")
        self.container_output_file = os.path.join(self.output_path, "container.json")
    ...
    @patch("src.container_drift.json.loads")
    def test_read_container_explorer_output_success(
        self, mock_json_loads, mock_path_exists
    ):
        """Tests read_container_explorer_output for success."""
        mock_path_exists.return_value = True
        mock_json_loads.return_value = [{"drift": "test"}]

        mock_open_file = mock_open(read_data='[{"drift": "test"}]')
        with patch("builtins.open", mock_open_file):
            result = read_container_explorer_output(self.container_output_file)
            self.assertEqual(result, [{"drift": "test"}])
    ...
    def test_container_drift(
        ...
        """Tests container drift function."""
        mock_get_input_files.return_value = self.input_files
        mock_subprocess_run.return_value = MagicMock(returncode=0, status="success")
        mock_path_exists.return_value = True

        with patch(
            "src.container_drift.read_container_explorer_output",
            return_value=[{"drift": "test"}],
        ):
            result = container_drift(
                pipe_result=None,
            ...
            expected_display_names = ["container_drift.json", "container_drift.log"]
